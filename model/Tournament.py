...
class Tournament:
    ...
    #Consstructor assumes number of runds as parameter
    ...
    def count_Buholtz(self, playerID):
        ...
        for i in range(len(self.__params_list)):
            if self.scores[i][playerID] != -1:
                sum += self.count_scores(self, playerID)
        return sum


    #Sorts tuple by given element

    def Sort_Tuple(tup, elem): 
    ...
    def get_scores(self):
        points = []
        for i in range(len(self.__params_list)):
            points.append((i, self.count_scores(self, i), self.count_Buholtz(self,i)))
        self.Sort_Tuple(points, 1)
        for i in range(len(self.__params_list)-1):
            if points[i][1] == points[i+1][1]:
                if self.count_Buholtz(self, i+1) > self.count_Buholtz(self, i):
